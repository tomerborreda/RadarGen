...
def build_dataset(cfg, resolution=224, **kwargs):
    ...
    # Extract dataset name if available (for RadarGenDatasetWrapper)
    extra = cfg.get("extra", {})
    dataset_name = extra.get("dataset_name", None) if isinstance(extra, dict) else None

    # Build log message with dataset name if available
    if dataset_name:
        logger.info(f"Constructing dataset {dataset_type} ({dataset_name})...")
    else:
    ...
    # Build completion message with dataset name if available
    if dataset_name:
        logger.info(
            f"{colored(f'Dataset {dataset_type} ({dataset_name}) constructed: ', 'green', attrs=['bold'])}"
